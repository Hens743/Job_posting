...
def scrape_job_portal(url, keywords):
    ...
    job_data = []
    for job in jobs:
        title = job.find("h2", class_="job-title").text.strip()  # Modify selector as needed
        description = job.find("p", class_="job-description").text.strip()  # Modify selector as needed
        link = job.find("a")["href"]  # Assuming job details are in an anchor tag
